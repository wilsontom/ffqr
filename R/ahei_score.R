...
ahei_score <- function(x, group)
{
  ...
  if (group == 'A7') {
    score_index <-
      score_increments(start = 0.01, end = 249.99, n = 900) %>% mutate(Score = rev(Score))
  ...
  if (group == 'A8') {
    score_index <-
      score_increments(start = 2.01, end = 9.99, n = 900) %>% mutate(Score = rev(Score))
